...
export default function ChatSidebar({ onSelectChat }: ChatSidebarProps) {
  ...
  // Get the stored username (unique identifier)
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Set loggedInUserFullName based on stored username
        if (storedUsername) {
          const currentUser = formattedUsers.find((user: User) => user.username === storedUsername);
          if (currentUser) {
            setLoggedInUserFullName(currentUser.name);
          }
        }
  ...
  return (
    <div className="w-1/4 bg-gray-200 border-r h-full overflow-y-auto flex flex-col">
      ...
      {/* Display logged-in full name and profile icon above the "Create New Group" button */}
      {loggedInUserFullName && (
        <div className="p-4 flex items-center border-b bg-white">
          ...
          />
          <span className="text-gray-800 font-semibold">{loggedInUserFullName}</span>
        </div>
      )}
